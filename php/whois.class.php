...
class Whois{
  ...
  public function whoislookup($domain)
  {
    ...
    {
      return self::queryWhois('whois.lacnic.net',$domain);
    }
    else if(preg_match("/^([-a-z0-9]{2,100})\.([a-z\.]{2,8})$/i",$domain))
    {
      ...
      $res = self::queryWhois($server, $domain);
  ...
  private static function queryWhois($server,$domain)
  {
    $out = '';
  ...
  public static function GetExpirationDate($data)
    ...
    {
    	$substr = @substr($match[3][0], 0, -1);
    	return trim($substr);
    }
    return null;
	}

  public static function GetUpdatedDate($data)
	{
		if(preg_match_all('/(Updated Date|Last updated): (.+)/', $data, $match))
    {
      $substr = @substr($match[2][0], 0, -1);
    	return trim($substr);
    }
    return null;
	}
	...
	{
		if(preg_match_all('/(Creation Date|Registered on): (.+)/', $data, $match))
    {
      $substr = @substr($match[2][0], 0, -1);
    	return trim($substr);
    }
  ...
  public static function GetRegistrar($data)
	{
    if(self::isNominet($data)) {
      if(preg_match_all('/(.+) \[Tag = (.+)\]/', $data, $match))
      {
      ...
      return null;
		...
		if(preg_match_all('/Registrar: (.+)/', $data, $match))
    {
      $substr = @substr($match[1][0], 0, -1);
    	return trim($substr);
    }
    return null;
	}

	public static function GetWhoisServer($data)
	{
		if(preg_match_all('/(Whois|WHOIS) Server: (.+)/', $data, $match))
    {
      $substr = @substr($match[2][0], 0, -1);
    	return trim($substr);
    }
  ...
  public static function GetIpStag($data)
	{
		if(preg_match_all('/(.+) \[Tag = (.+)\]/', $data, $match))
    {
    	return trim($match[2][0]);
    }
    return null;
	}

  public static function isNominet($data)
  {
    return preg_match('/Nominet/', $data);
  }

  private static function parseNominet($data)
  {
